...
void ListaDisparos::destruir_contenido() {
    for (auto d : lista) delete d;
    lista.clear();
}
...
Disparo* ListaDisparos::operator[](int index) {
    if (index >= lista.size())index = (int)(lista.size()) - 1;
    if (index < 0)index = 0;
    return lista[index];
}

void ListaDisparos::colision(const Bordes& b) {
    for (int i = (int)lista.size() - 1; i >= 0; i--)
        if (InteraccionArena::colision(*lista[i], b))
            eliminar(i);
}
